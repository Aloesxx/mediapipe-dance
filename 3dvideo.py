...
def process_frame(img):
    ...
    if results.pose_landmarks:  # 若检测出人体关键点
        ...
        for i in range(33):  # 遍历33个关键点
            cx = results.pose_world_landmarks.landmark[i].x
            cy = results.pose_world_landmarks.landmark[i].y
            cz = results.pose_world_landmarks.landmark[i].z
            radius = 3
...
def generate_vedio(input_path, file_type='mp4'):
    ...
    output_path = domAddress + '/3dout-' + file_head
